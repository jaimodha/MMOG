...
public class Database {
	...
	public void startConnection() throws Exception {
		try {
			...
			connect = DriverManager.getConnection(
					"jdbc:mysql://localhost/gamedev", "gamedev", "3tb=PChh");
	...
	public boolean CreateCharacter(Integer char_type, String char_name,String username, Integer t_id) {
		...
		int uid = getuid(username);
		   ...
		   try {
			   if (CanCreateCharacter(username) == true){
	...
	public boolean CanCreateCharacter(String uname ) {
		boolean result = true;
		int uid = getuid(uname);
		ArrayList<String> s = new ArrayList<String>();
		try {
			preparedStatement = connect.prepareStatement("SELECT * FROM characters WHERE u_id=?");
			preparedStatement.setInt(1, uid);
			ResultSet rs = preparedStatement.executeQuery();
			while (rs.next()) {
				s.add(rs.getString("char_name"));
			}
			if (s.size() >= 3) {
				result = false;
			}
	...
	public ArrayList<CharacterModel> getCharacterfromUser(String uname) {
		...
		try {
			...
			ResultSet rs = preparedStatement.executeQuery();
			while (rs.next()) {
				int tn = rs.getInt("t_id");

				c.add(new CharacterModel(rs.getInt("char_id"), rs
						.getInt("char_type"), uname, tn,rs.getString("char_name")));
	...
	public int getcharId(int userid,String charname,int ctype,int cfac)
	{
		int cid=0;
		try {
			preparedStatement = connect
					.prepareStatement("SELECT * FROM characters WHERE u_id=? and char_name=? and char_type=? and t_id=?");
			preparedStatement.setInt(1,userid);
			preparedStatement.setString(2,charname);
			preparedStatement.setInt(3,ctype);
			preparedStatement.setInt(4,cfac);
			ResultSet rs = preparedStatement.executeQuery();
			if (rs.next()) {
				cid=rs.getInt("char_id");
			}
		} catch (Exception e) {
