...
public class RequestCharacterCreation extends GameRequest{
	...
	public void doBusiness() throws Exception {
		...
		{
			 cr.setFlag(1);
			 System.out.println(client.getUm().getUsername());
			 /////
			CharacterModel ch = new CharacterModel(cr_type,cr_func,cr_name,true); 
			client.getUm().getCharlist().add(ch);
			cr.setCharcount(client.getUm().getCharlist().size());
			cr.setCharfaction(cr_func);
			int cid = db.getcharId(client.getUm().getUserid(), cr_name, cr_type, cr_func);
			cr.setCharid(cid);
			cr.setCharname(cr_name);
			cr.setChartype(cr_type);
         	/////
		}
		else
		{
