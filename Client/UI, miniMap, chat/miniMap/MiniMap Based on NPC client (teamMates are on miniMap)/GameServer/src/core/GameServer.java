...
//import networking.response.ResponsePanda;
...
public class GameServer {
    ...
    //private Panda panda = new Panda(gameServer);
    ...
    private void run() {
        ...
        try {
            ...
            while (true) {
                ...
                /*panda.updatePanda();
                ResponsePanda response = new ResponsePanda();
                response.setPanda(panda);
                addResponseForAllOnlinePlayers(-1, response);*/
    ...
    public GameClient getThreadByPlayerID(int playerID) {
        for (GameClient aClient : activeThreads.values()) {
            if (aClient.getPlayer().getPlayer_id() == playerID) {
                return aClient;
            }
        }
    ...
    public void addResponseForAllOnlinePlayers(long player_id, GameResponse response) {
        for (GameClient client : activeThreads.values()) {
            if (client.getPlayer().getPlayer_id() != player_id) {
                client.addResponseForUpdate(response);
            }
        }
    }
