...
class ControlPoint():
    ...
    def __init__(self, id, x, y, z, radius, factionId):
        ...
        self.model = loader.loadModel("models/cparea")
    ...
    def checkRedPresence(self, characters):
        for x in characters.values():
            if self.withinCircle(x.get_character()) and x.get_team() == RED:
                return True
        return False
    ...
    def checkBluePresence(self, characters):
        for x in characters.values():
            if self.withinCircle(x.get_character()) and x.get_team() == BLUE:
                return True
        return False
